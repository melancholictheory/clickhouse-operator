...
You can attach a new disk to a cluster that already holds data by appending an entry
to `additionalVolumeClaimTemplates`. The new disk joins the JBOD storage policy, and
ClickHouse starts placing new parts on it.

A StatefulSet's `volumeClaimTemplates` are immutable, so the operator recreates the
StatefulSet to attach the disk. PersistentVolumeClaims are retained through that
recreation and the recreated StatefulSet asks for the same claim names, so existing
disks keep their data and only the new claim is provisioned.

The pod of each replica restarts once as part of this. Replicas are processed one at
a time, the same way a rolling configuration change is, so a multi-replica shard stays
available throughout.

<Note>
Declaring a disk through `settings.extraConfig` without a matching
`additionalVolumeClaimTemplates` entry does not give you a separate volume. ClickHouse
registers the disk and `system.disks` reports it, but its path is a directory on the
main data volume, so parts land there instead. Compare `total_space` against the main
volume to spot it.
</Note>
...
- The presence of `dataVolumeClaimSpec` is immutable — you cannot **add** a data
  ...
  with one.
- Entries in `additionalVolumeClaimTemplates` cannot be **removed** or **renamed**
  after creation, because the data on those disks would be orphaned. Adding a new
  entry **is** allowed, see [Adding a disk](#adding-a-disk).
...
| Removing or renaming `additionalVolumeClaimTemplates` after creation | Rejected |
| Adding an entry to `additionalVolumeClaimTemplates` after creation | Accepted, replicas are recreated one at a time |
